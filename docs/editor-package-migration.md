...
将 `markdown_visual_editor` 的编辑器代码打包为 npm 包 `@chengxinsun26/editor`，i_blog 通过 `pnpm install` 引用。与其他第三方包一样的使用方式。
...
- editor 包导出 TypeScript 源码（.ts），i_blog 的 `transpilePackages` 负责编译
