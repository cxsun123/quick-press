...
i_blog 支持 [Model Context Protocol (MCP)](https://modelcontextprotocol.io/)，允许 AI 客户端通过 MCP 协议管理文章。
...
```bash
# 构建
docker build -t i-blog .

# 运行（自定义端口）
docker run -p 3000:3000 --env-file .env i-blog
```
